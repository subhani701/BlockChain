...
export interface DataTableColumn<T> {
  ...
  /** Shared width / visibility classes on header + body cells */
  className?: string;
  /** Header-only classes (alignment, padding) */
  headerClassName?: string;
  /** Body cell-only classes (alignment, padding) */
...
interface DataTableProps<T> {
  ...
  embedded?: boolean;
  /** Show entry count footer (reference table style) */
  showFooter?: boolean;
  page?: number;
  pageSize?: number;
  /** Column width strategy — use fixed when columns declare explicit widths */
...
export function DataTable<T>({
  ...
  const wrapperProps = embedded
    ? { className: cn('h-fit w-full overflow-hidden', className) }
    : {
        variants: fadeInUp,
        initial: 'initial' as const,
        animate: 'animate' as const,
    ...
    <Wrapper {...wrapperProps}>
      ...
      {showDesktopTable && (
        <div className={cn('ds-table-wrap', showMobileCards && layout === 'auto' && 'hidden md:block')}>
          <table className={cn('w-full min-w-full text-sm', tableLayout === 'fixed' ? 'table-fixed' : 'table-auto')}>
            <thead>
              <tr>
      ...
      {showFooter && total > 0 && (
        <div className="ds-table-footer">
          <span className="ds-table-footer-meta">
