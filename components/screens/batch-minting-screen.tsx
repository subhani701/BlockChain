    ...
    () => [
        ...
        cell: (batch: Batch) => (
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <TableCellCode>{batch.id}</TableCellCode>
      ...
      },
      {
        ...
        cell: (batch: Batch) => {
          ...
          return (
            <TableCellStack
              primary={part?.name ?? batch.partId}
              secondary={[part?.sku, part?.category].filter(Boolean).join(' · ')}
            />
          );
        },
      },
      {
        ...
        hideOnMobile: true,
        cell: (batch: Batch) => (
          <TableCellStack primary={batch.plant} secondary={`Line ${batch.line}`} />
        ),
      },
      {
        key: 'operator',
        ...
        hideOnMobile: true,
        cell: (batch: Batch) => (
          <TableCellStack primary={batch.operator} secondary={`QA · ${batch.qaInspector}`} />
        ),
      },
      {
        key: 'units',
        ...
        cell: (batch: Batch) => (
          <span className="text-xs font-semibold tabular-nums text-foreground">{batch.unitCount.toLocaleString()}</span>
        ),
      },
      {
        key: 'mfg',
        ...
        hideOnMobile: true,
        cell: (batch: Batch) => (
          <span className="text-[11px] text-muted-foreground whitespace-nowrap">{batch.mfgWindow}</span>
        ),
      },
      {
        key: 'priority',
      ...
      },
      {
        ...
        hideOnMobile: true,
        cell: (batch: Batch) => (
          <span className="text-[11px] text-muted-foreground">{formatBatchSyncAge(batch.sapSyncedAt)}</span>
        ),
      },
      {
        key: 'qa',
    ...
    <div className="ds-page">
      <motion.div initial="initial" animate="animate" variants={staggerContainer} className="ds-page-inner-wide ds-stack">
        <motion.div variants={fadeInUp}>
          <PageHeader
            icon={Layers}
            title="Batch Minting Console"
            subtitle={`Anchor SAP batches to ${VOLTUS_CHAIN_LABEL}`}
            action={
              <div className="ds-contract-pill">
                ...
                <span className="text-foreground font-mono text-[11px] sm:text-xs break-all">{formatHexDisplay(PROVENANCE_REGISTRY_ADDRESS)}</span>
              </div>
            }
          />
        </motion.div>
                  ...
                  <div className="ds-filter-bar">
                    {(
                      ...
                      <button
                        ...
                        className={`ds-filter-pill ${batchFilter === f.id ? 'ds-filter-pill-active' : 'ds-filter-pill-inactive'}`}
                      >
