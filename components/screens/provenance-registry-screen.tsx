...
import { useBatchAnchoring, useProvenanceRegistry } from '@/components/providers/app-provider';
import {
  DAO_MINT_QUORUM,
...
const ADMIN_ROLES: { id: ProvenanceAdminRole; label: string; canWrite: boolean }[] = [
  { id: 'plant-admin', label: 'Plant Admin', canWrite: true },
  { id: 'quality-admin', label: 'Quality Admin', canWrite: true },
  { id: 'oem-viewer', label: 'OEM Admin (read-only)', canWrite: false },
];
...
export function ProvenanceAdminScreen({ callbacks }: ProvenanceRegistryProps) {
  ...
  const [adminRole, setAdminRole] = useState<ProvenanceAdminRole>('plant-admin');
  ...
  const canWrite = ADMIN_ROLES.find((r) => r.id === adminRole)?.canWrite ?? false;
  ...
  const visibleIdentities = useMemo(
    () => filterProvenanceIdentities(identityRows, identityFilter),
    [identityRows, identityFilter],
  );
    ...
    () => [
      {
        key: 'did',
        header: 'DID',
        cell: (row) => <code className="ds-mono-link text-xs">{row.did}</code>,
      },
      {
        key: 'name',
        header: 'Organization',
      ...
      },
      {
        ...
        header: 'VCs',
        hideOnMobile: true,
        cell: (row) => <span className="tabular-nums">{row.credentials}</span>,
      },
      {
        key: 'actions',
        header: 'Actions',
        ...
        cell: (row) => (
          <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              className="ds-btn-sm ds-btn-outline disabled:opacity-40"
            >
      ...
      >
        <motion.div variants={fadeInUp} className="ds-page-header">
          <div>
            ...
            <p className="ds-subtitle">DIDs, verifiable credentials, DAO governance, and on-chain registry</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <select
                value={adminRole}
                onChange={(e) => {
                  setAdminRole(e.target.value as ProvenanceAdminRole);
                  callbacks.showToast(`Switched to ${e.target.selectedOptions[0].text}`, 'info');
                }}
          ...
          <motion.div variants={fadeInUp} className="lg:col-span-2 space-y-4">
            ...
            >
              <div className="flex flex-wrap gap-2 mb-3">
                {(
                  ...
                  <button
                    ...
                    className={`ds-filter-chip text-[11px] ${identityFilter === f.id ? 'ds-filter-chip-active' : ''}`}
                  >
                    {f.label}
                  </button>
                ))}
              ...
              <DataTable
                ...
                renderExpanded={(row) => (
                  <div className="px-4 py-3 bg-muted/20 text-xs space-y-1">
                    <p>
                      <span className="font-semibold text-foreground">DID resolution: </span>
                      <code className="font-mono">{row.did}</code>
                    </p>
                    <p className="text-muted-foreground">
                      {/* prov__did_registry read — VC issuance binds to governance__vc_policy */}
              ...
              />
              <ProvenanceHint source="prov__did_registry" freshness={`${visibleIdentities.length} identities · plants / distributors / service`} className="mt-3" />
            </DataCard>
          </motion.div>
            ...
            >
              ...
              <div className="space-y-1.5 mb-3 max-h-28 overflow-y-auto">
                {daoMembers.map((m) => (
                  <div key={m.address} className="flex justify-between text-xs gap-2">
                    <code className="font-mono text-muted-foreground truncate">{m.address}</code>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {(['all', 'pending', 'approved'] as const).map((f) => (
                  <button
                    ...
                    className={`ds-filter-chip text-[11px] ${proposalFilter === f ? 'ds-filter-chip-active' : ''}`}
                  >
                    {f}
                  </button>
                ))}
              ...
              <div className="space-y-2">
                {visibleProposals.map((proposal) => (
                  <div key={proposal.id} className="p-3 rounded-lg border border-border bg-muted/20">
              ...
              <ProvenanceHint source="governance__dao_vote" freshness="approvals gate Batch Minting anchor policy" className="mt-3" />
            </DataCard>
            ...
            >
              <div className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <p className="font-mono text-primary mt-0.5">{registry.address}</p>
                </div>
              ...
              <p className="text-xs text-muted-foreground mt-3">
                {/* ProvenanceRegistry.sol read/write — anchorBatch() persists 32-byte Merkle roots only */}
                Registry read/write binds to ProvenanceRegistry.sol; only roots are stored on-chain.
              </p>
              <ProvenanceHint source="ProvenanceRegistry.sol" freshness={`v${registry.version.replace('v', '')} deployed`} className="mt-2" />
            </DataCard>
          </motion.div>
        </div>
      </motion.div>
              ...
              <div className="ds-modal-body">
                {registrationComplete ? (
                  <div className="text-center py-6 space-y-3">
                    ...
                    <code className="ds-chip ds-chip-primary block break-all">{generatedDid}</code>
                    <button type="button" onClick={resetRegistration} className="ds-btn-md ds-btn-primary">Done</button>
                  </div>
                ) : registerStep === 1 ? (
